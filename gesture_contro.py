...
def create_trackbars():
    ...
    cv2.createTrackbar('SatMin', 'Trackbars', 28, 255, empty)
    cv2.createTrackbar('SatMax', 'Trackbars', 80, 255, empty)
...
while(1):
    ...
    # print(frame.shape)
    frame = frame[190:, 300:] # only considering frame from row 0-300 and col from 300-end so that main focus is on our hands
    ...
    if(centroid_x,centroid_y) != (-1,-1):
        ...
        hand_detected = cv2.contourArea(max_cntr) >= 20000 # Max contour area must be greater than thois threshold 
